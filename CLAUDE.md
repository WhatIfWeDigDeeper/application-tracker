...
- **protoPath at runtime**: Use `join(process.cwd(), 'proto', 'history', 'v1', 'history.proto')` — both services are run from the repo root, so `process.cwd()` resolves correctly. Do not use `__dirname`-relative paths that assume the compiled dist directory structure.
...
- **Cross-schema cascade delete**: When an application is deleted, `nest-api` explicitly calls `historyClient.deleteHistory(applicationId)` before deleting the application row. There is no cross-schema FK cascade since history lives in a separate schema (`react_nestjs_history`) owned by `nest-history-api`.
