...
**`bundleDependencies` defeats `overrides`** — bundled tarballs ship their own copy of nested deps that ignore root overrides; bump the parent (e.g. `aws-cdk-lib` `2.248.0` → `2.254.0` to drop a bundled vulnerable `fast-uri`). Then run a full `npm install` (not `--package-lock-only`) so bundled-nested entries land in the lockfile — otherwise CI's `npm ci` fails with `Missing: <pkg> from lock file`.
