...
import { useApplicationHistory } from "../../queries/applicationQueries";
import { useRestoreToVersion } from "../../queries/applicationMutations";
...
export function HistoryPanel({
  ...
  const { data, isLoading: loading, error: queryError } = useApplicationHistory(applicationId);
  ...
  async function handleRestore(entry: HistoryEntry) {
    try {
      await restoreMutation.mutateAsync({ id: applicationId, sequence: entry.sequence });
      onRestored();
    } catch {
                ...
                {expandedEntry === entry.id && (
                  <div className="px-3 pb-3 space-y-3">
                    ...
                    {i > 0 && (
                      ...
                      >
                        {restoreMutation.isPending ? "Restoring..." : "Restore to this point"}
                      </button>
                    )}
                  </div>
                )}
