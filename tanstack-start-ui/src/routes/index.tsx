...
import { fetchApplications } from "../server/applications";
import { queryClient } from "../lib/queryClient";
import { applicationKeys } from "../queries/queryKeys";

// Must match useApplications() initial state (useFilters + useSorting defaults).
// undefined fields are excluded from TanStack Query's key hash, so omitting them here is equivalent.
...
export const Route = createFileRoute("/")({
  loader: () =>
    queryClient.prefetchQuery({
      queryKey: applicationKeys.list(DEFAULT_LIST_PARAMS as Record<string, unknown>),
      queryFn: () => fetchApplications({ data: DEFAULT_LIST_PARAMS }),
    }),
  component: ListPage,
});
