...
- `nest-api/src/applications/applications.controller.ts` — inject `HistoryClient` instead of `HistoryService` for both `GET /applications/:id/history` and `POST /applications/:id/history/restore`. The restore endpoint delegates to `historyClient.restoreToVersion(id, sequence)`, which calls `getSnapshotAtVersion` over gRPC then applies the snapshot to `applications` + `interview_stages` inside a single Drizzle transaction. (The implementation consolidates restore logic in `HistoryClient` rather than adding a separate `ApplicationsService.restoreToVersion` method — the write-isolation boundary is still clean since the gRPC service never writes non-history tables.)
