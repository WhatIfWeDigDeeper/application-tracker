# Feature Specification: Application History & Restore (React + Koa/Raw SQL)
...
The svelte-hono implementation has snapshot-based history with Drizzle, and vue-nuxt uses event sourcing with Immer patches. This spec covers adding the same **snapshot-based** history feature to the React + Koa/Raw SQL stack.
...
1. **Database**: New `application_history` table in `react_koa` schema (raw SQL DDL)
...
6. **No New Dependencies**: Uses existing pg, Koa, Zod, and React tooling
...
- [ ] Build and lint pass for both koa-api and react-ui
