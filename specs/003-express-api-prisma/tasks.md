...
- [ ] T001 Create `ui/` directory and move Next.js app files from root to ui/
- [ ] T002 Move `src/` to `ui/src` and `public/` to `ui/public`
- [ ] T003 Move configs to `ui/`: `package.json`, `tsconfig.json`, `next.config.mjs`, `postcss.config.mjs`, `tailwind.config.ts`, `jest.config.js`, `jest.setup.js`, `playwright.config.ts`
- [ ] T004 [P] Add `ui/.env.example` based on existing envs
- [ ] T005 [P] Add `ui/Dockerfile` (multi-stage Next.js build)
- [ ] T006 Create `api/` directory with base structure: `src/`, `prisma/`, `package.json`, `tsconfig.json`, `.env.example`, `Dockerfile`
- [ ] T007 [P] Configure root `.github/dependabot.yml` to track `/ui`, `/api`, and root docker files
- [ ] T008 [P] Add root `.env.example` with compose envs (e.g., `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB`)
...
- [ ] T009 Setup Prisma in `api`: add `@prisma/client` and `prisma` dependencies in `api/package.json`
- [ ] T010 [P] Create `api/prisma/schema.prisma` aligning with `data-model.md`
- [ ] T011 [P] Add `api/src/db/client.ts` to instantiate PrismaClient and export singleton
- [ ] T012 [P] Add `api/src/middleware/errorHandler.ts` and `api/src/middleware/logger.ts`
- [ ] T013 [P] Implement Express bootstrap in `api/src/index.ts` (load env, middleware, health route, router mount)
- [ ] T014 Configure `docker-compose.yml` with services: `postgres`, `api`, `ui`, volumes and healthchecks
- [ ] T015 [P] Add `api` scripts: `prisma generate`, `prisma migrate dev`, `prisma migrate deploy`, `prisma:seed`
...
- [ ] T019 [P] [US1] Contract tests for `/applications` endpoints in `api/tests/contract/applications.contract.test.ts`
- [ ] T020 [P] [US1] Integration tests for applications workflow in `api/tests/integration/applications.integration.test.ts`

### Implementation

- [ ] T021 [P] [US1] Implement zod DTOs in `api/src/types/applications.dto.ts`
- [ ] T022 [P] [US1] Implement `Application` service in `api/src/services/applications.service.ts` using PrismaClient
- [ ] T023 [US1] Implement router in `api/src/routes/applications.ts` (list, create, get, patch, delete)
...
- [ ] T027 [P] [US2] Contract tests for `/applications/{id}/interview-stages` in `api/tests/contract/stages.contract.test.ts`
- [ ] T028 [P] [US2] Integration tests for stage workflow in `api/tests/integration/stages.integration.test.ts`

### Implementation

- [ ] T029 [P] [US2] Implement zod DTOs in `api/src/types/stages.dto.ts`
- [ ] T030 [P] [US2] Implement `InterviewStage` service in `api/src/services/stages.service.ts`
- [ ] T031 [US2] Implement router in `api/src/routes/interview-stages.ts` (create, patch, delete)
- [ ] T032 [US2] Wire stage routes nested under application in `api/src/index.ts`
- [ ] T033 [US2] Cascade delete stages when application is deleted (service logic)
- [ ] T034 [US2] Seed sample stages in `api/src/db/seed.ts`
...
- [ ] T035 [P] [US3] Contract test for `/health` in `api/tests/contract/health.contract.test.ts`

### Implementation

- [ ] T036 [P] [US3] Implement `/health` route in `api/src/index.ts`
- [ ] T037 [US3] Ensure logger middleware applied globally
