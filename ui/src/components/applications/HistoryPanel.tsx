...
export function HistoryPanel({ applicationId, refreshKey, onClose, onRestored }: HistoryPanelProps): React.ReactElement {
  ...
  const loadHistory = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await applicationsApi.getHistory(applicationId);
    ...
    <div className="fixed inset-y-0 right-0 w-96 z-40 bg-white dark:bg-gray-800 shadow-xl border-l border-gray-200 dark:border-gray-700 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {loading ? (
          ...
          <p className="text-gray-500 dark:text-gray-400 text-center py-6">No history entries yet.</p>
        ) : (
              ...
              <div key={entry.id}>
                <button
                  ...
                  className={`w-full text-left p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 ${
                    expandedEntry === entry.id ? 'bg-gray-50 dark:bg-gray-700/50' : ''
                  }`}
                >
                ...
                {expandedEntry === entry.id && (
                  <div className="px-3 pb-3 space-y-3">
                    ...
                    {i > 0 && (
                      <button
                        type="button"
                        className="w-full text-sm text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium py-1.5 px-3 rounded-md border border-blue-200 dark:border-blue-800 hover:bg-blue-50 dark:hover:bg-blue-900/30"
                        disabled={restoring}
